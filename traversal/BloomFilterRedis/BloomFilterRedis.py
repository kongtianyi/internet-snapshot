...
import redis
from BloomFilterRedis import GeneralHashFunctions # 这里pycharm爆红，属于误报
...
class BloomFilterRedis:
    ...
    def __init__(self, key, host='172.0.0.1', port=6379, pwd="", hash_list=hash_list):
        # redis-bitmap的key
        self.key = key
        # redis连接信息
        self.pool = redis.ConnectionPool.from_url(url="redis://:%s@%s:%s/0" % (pwd, host, port))
        self.pwd = pwd
